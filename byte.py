...
# ==================== 配置区 ====================
JSON_FILE_PATH = "/Users/asuria/Desktop/browser/doubao.json"
...
def read_json_file(file_path: str) -> dict:
    ...
    send_type = data.get("sendType", "article")
    ...
    if send_type == "article" and not content:
        raise ValueError("article 类型未找到 content 字段")
...
async def main():
    ...
    try:
        ...
        if send_type == "article":
            ...
            print(f"❌ 不支持的发送类型: {send_type}")
            print(f"   支持的类型: article（文章）、image（图文）、video（视频）")
