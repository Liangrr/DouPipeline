...
> **一句话介绍：** 运行一条命令，AI 自动帮你写文案、生成图片、发布到抖音/小红书，还能自动生成小说发布到番茄小说。
>
...
| 📚 **番茄小说自动生成+发布** | AI 自动生成小说架构、逐章写正文，自动发布到番茄小说创作者后台 |
...
│                                                      │
│  Step 2: AI 逐章生成正文                              │
│    → 根据大纲自动写小说正文（每章 2000-5000 字）        │
│                                                      │
│  Step 3: 浏览器自动发布                               │
│    → 打开番茄小说创作者后台 → 创建书籍 → 逐章发布       │
└─────────────────────────────────────────────────────┘
```
...
| 📖 **小说** | 番茄小说 | AI 生成架构+逐章正文，自动发布到番茄小说后台 |

### 为什么用这个工具？

- 🚀 **省时** — 从写文案到发布全程自动，一条命令搞定
- 🎨 **出图质量高** — 提示词经过角色化优化，豆包直出效果好
- 📚 **小说自动写** — AI 生成架构+逐章写正文，自动发布到番茄小说
- ✂️ **自动去水印** — 不用手动处理，下载即用
...
- 需要能访问 **番茄小说创作者后台**（fanqienovel.com）—— 用于发布小说
- 需要能访问 **LLM API**（默认是小米 MiMo 接口）—— 用于 AI 生成文案/小说
...
uv run python novel_generator.py --topic 玄幻修仙 --book-dir 玄幻修仙 --only 4 --total 100

# 重新规划后，继续生成新章节
uv run python run.py --platform fanqie --book-dir 玄幻修仙 --only 2 --start 11


# ========== 中途崩溃？补全缺失大纲 ==========

# 自动检测缺失章节并补全（如1-100有，100-200丢失，200-300有 → 自动补100-200）
uv run python novel_generator.py --topic 玄幻修仙 --book-dir 玄幻修仙 --only 5
...
| `--only 1` | - | 只生成大纲（run.py） |
| `--only 2` | - | 只生成章节内容（run.py） |
| `--only 3` | - | 只发布到番茄小说（run.py） |
| `--step 2` | - | 从第2步开始连续执行：生成章节+发布 |

### novel_generator.py 参数速查表

> 💡 `novel_generator.py` 用于大纲的追加、重新规划、补全等高级操作，日常生成和发布用 `run.py` 即可。

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--topic` | - | 小说题材（必填） |
| `--book-dir` | - | 书名（自动定位到账号目录） |
| `--only 1` | - | 生成小说架构（同 run.py --only 1） |
| `--only 2` | - | 生成章节内容（同 run.py --only 2） |
| `--only 3` | - | 追加新大纲 + 自动生成内容（加更） |
...
```bash
uv run python novel_generator.py --topic 玄幻修仙 --book-dir 玄幻修仙 --only 3 --add 5
```
